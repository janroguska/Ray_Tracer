...
void	get_angle(t_master *m, t_light *l, t_shape *s)
{
	...
	if (len != 0.0)
	{
		light_to_shape_x /= len;
		light_to_shape_y /= len;
		light_to_shape_z /= len;
	}
	else
	{
		light_to_shape_x = 0.0;
		light_to_shape_y = 0.0;
		light_to_shape_z = 0.0;
	}
	if (l->ambience > 1.0)
		l->ambience = 1.0;
	if (l->intensity > 1.0)
	...
	if (s->shape_id != 3)
	{
		m->l.angle = (light_to_shape_x * m->l.light_normal_x)
		+ (light_to_shape_y * m->l.light_normal_y)
		+ (light_to_shape_z * m->l.light_normal_z);
	}
	else
	{
		m->l.angle = fabs((light_to_shape_x * m->l.light_normal_x)
		+ (light_to_shape_y * m->l.light_normal_y)
		+ (light_to_shape_z * m->l.light_normal_z));
	}
